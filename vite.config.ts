    ...
    plugins: [
      ...
      mediaCdnPlugin(mediaCdnOrigin, mediaCdnStripPrefix, videoCdnOrigin, videoCdnStripPrefix),
      {
        ...
        transformIndexHtml(html: string) {
          const optimizedBase = buildOptimizedMediaBase(mediaCdnOrigin, mediaCdnStripPrefix);
          let next = html.replaceAll('%SITE_ORIGIN%', siteOrigin);
          next = next.replaceAll('%MEDIA_OPTIMIZED_BASE%', optimizedBase);

          if (mediaCdnOrigin) {
            const normalized = normalizeMediaCdnOrigin(mediaCdnOrigin);
            const cdnOrigin = new URL(
              buildMediaCdnUrl('/images/optimized/DSC01989_960.webp', normalized, mediaCdnStripPrefix),
            ).origin;
            if (!next.includes('rel="preconnect"')) {
              next = next.replace(
                '<head>',
                `<head>\n    <link rel="preconnect" href="${cdnOrigin}" crossorigin />`,
              );
            }
            next = next.replace(/href="(\/images\/[^"]+)"/g, (_match, assetPath: string) => {
              return `href="${buildMediaCdnUrl(assetPath, normalized, mediaCdnStripPrefix)}"`;
            });
          }
