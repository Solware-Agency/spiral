...
export default async function handler(req, res) {
  ...
  if (!sessionId) {
    return json(res, 400, { ok: false, error: 'Missing Stripe session ID' });
  }
  ...
  try {
    ...
    if (!session || session.mode !== 'payment') {
      return json(res, 400, { ok: false, error: 'Invalid checkout session' });
    }
    if (session.payment_status !== 'paid') {
      return json(res, 402, { ok: false, error: 'El pago no está completado.' });
    }
    ...
    const plan =
      md.bookingPlan === 'weekend'
        ? 'weekend'
        : md.bookingPlan === 'weekday'
          ? 'weekday'
          : null;
    ...
    if (!plan || !Number.isFinite(hours) || hours < 1 || hours > 12 || !date || !time) {
      return json(res, 400, { ok: false, error: 'Booking metadata is invalid in Stripe session' });
    }

    const dm = date.match(/^(\d{4})-(\d{2})-(\d{2})$/);
    if (!dm) return json(res, 400, { ok: false, error: 'Invalid booking date format' });
    const year = Number(dm[1]);
    const month = Number(dm[2]);
    const day = Number(dm[3]);

    const t = parseTime12hTo24h(time);
    if (!t) return json(res, 400, { ok: false, error: 'Invalid booking time format' });
    ...
    if (!start.isValid || !end.isValid) {
      return json(res, 400, { ok: false, error: 'Invalid booking date/time' });
    }
    if (end > bookingClose) {
      return json(res, 400, {
        ok: false,
